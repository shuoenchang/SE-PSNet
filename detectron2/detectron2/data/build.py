...
def print_instances_class_histogram(dataset_dicts, class_names):
    ...
    num_classes = len(area_names)
    hist_bins = [0, 32**2, 96**2, 1024**2]
    histogram = np.zeros((3,), dtype=np.int)
    # min_=1024**2
    # max_=0
    # avg_=0
    for entry in dataset_dicts:
        annos = entry["annotations"]
        areas = [x["area"] for x in annos if not x.get("iscrowd", 0)]
        # min_ = min_ if min_<min(areas) else min(areas)
        # max_ = max_ if max_>max(areas) else max(areas)
        # avg_ += sum(areas)
        histogram += np.histogram(areas, bins=hist_bins)[0]

    # print(min_, avg_/849949, max_)
    N_COLS = min(6, len(class_names) * 2)
    ...
    data = list(
        itertools.chain(*[[short_name(area_names[i]), int(v)] for i, v in enumerate(histogram)])
    )
    total_num_instances = sum(data[1::2])
    if num_classes > 1:
        data.extend(["total", total_num_instances])
