...
def load_coco_panoptic_json_eval(json_file, image_dir, gt_dir, meta):
    """
    Args:
        image_dir (str): path to the raw dataset. e.g., "~/coco/train2017".
        gt_dir (str): path to the raw annotations. e.g., "~/coco/panoptic_train2017".
...
def register_coco_panoptic(
    ...
    if panoptic_root is not None:
        ...
        DatasetCatalog.register(
            panoptic_name,
            lambda: load_coco_panoptic_json_eval(panoptic_json, image_root, panoptic_root, metadata),
        )
    MetadataCatalog.get(panoptic_name).set(
        ...
        evaluator_type="coco_panoptic_seg_no_sem",
        ignore_label=255,
        label_divisor=1000,
        **metadata,
    )
    ...
    if panoptic_root is not None:
        DatasetCatalog.register(
            ...
            lambda: merge_to_panoptic(
                load_coco_json(instances_json, image_root, panoptic_name, extra_annotation_keys=['area']),
