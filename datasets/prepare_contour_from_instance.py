...
def _process_instance_to_semantic(anns, output_semantic, img, categories):
    ...
    output = torch.zeros(img_size)

    for ann in anns:
        mask = annToMask(ann, img_size)
        per_instance = torch.tensor(mask, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
        per_instance_contour = F.conv2d(per_instance, laplacian_kernel, padding=1)
        per_instance_contour[per_instance_contour > 0] = 1
        per_instance_contour[per_instance_contour <= 0] = 0
        contour = torch.sum(per_instance_contour, dim=(0, 1)).squeeze()
        output += contour
    output[output > 0] = 1
    output[output <= 0] = 0
    output = output.numpy()
    # output_img = output_semantic.replace('contour_train2017', 'contour_train2017_img').replace('npz', 'png')
...
if __name__ == "__main__":
    ...
    for s in ["train2017"]:
        create_coco_semantic_from_instance(
            ...
            os.path.join(dataset_dir, "contour_{}".format(s)),
