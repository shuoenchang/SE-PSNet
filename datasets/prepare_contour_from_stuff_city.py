...
def _process_stuff_to_semantic(img_path, output_path):
    ...
    output_img = output_path.replace('contour_stuff_train', 'contour_stuff_train_img').replace('npz', 'png')
    ...
    os.makedirs(output_dir.replace('contour_stuff_train', 'contour_stuff_train_img'), exist_ok=True)
...
def create_coco_contour_from_semantic(data_root, output_root):
    ...
    instance_gt_paths = glob.glob(os.path.join(data_root, "*", "*_gtFine_labelStuffIds.png"))

    def iter_annotations():
        for gt_path in tqdm(instance_gt_paths):
            img_path = gt_path
            output = gt_path.replace('gtFine_labelStuffIds.png', 'leftImg8bit.npz').replace('train', 'contour_stuff_train')
...
if __name__ == "__main__":
    ...
    for s in ["train2017"]:
        create_coco_contour_from_semantic(
            ...
            os.path.join(dataset_dir, "gtFine", "contour_stuff_train")
        )
