...
class SEPSNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.instance_loss_weight = cfg.MODEL.SEPSNET.INSTANCE_LOSS_WEIGHT
        ...
        attn_size = cfg.MODEL.SEPSNET.ATTN_SIZE
    ...
    def forward(self, batched_inputs):
        ...
        if self.combine_on:
            ...
            if "stuff_contour" in batched_inputs[0]:
                stuff_contour = [x["stuff_contour"].to(self.device) for x in batched_inputs]
                stuff_contour = ImageList.from_tensors(
                    stuff_contour, self.backbone.size_divisibility, 0).tensor
            else:
                stuff_contour = None
            sem_seg_results, sem_seg_losses, silhouette_stuff = self.panoptic_module(features, gt_sem, stuff_contour)
        ...
        if "basis_contour" in batched_inputs[0]:
            basis_contour = [x["basis_contour"].to(self.device) for x in batched_inputs]
            basis_contour = ImageList.from_tensors(
                basis_contour, self.backbone.size_divisibility, 0).tensor
        else:
            basis_contour = None

        basis_out, basis_losses = self.basis_module(features, basis_sem, basis_contour, gt_instances)
        proposals, proposal_losses = self.proposal_generator(
            images, features, gt_instances, self.top_layer)
        detector_results, detector_losses = self.blender(
            basis_out["bases"], proposals, gt_instances, features["p3"])
        ...
        for i, (detector_result, input_per_image, image_size) in enumerate(zip(
            ...
            if self.basis_module.visualize:
                if self.combine_on:
                    processed_result["silhouette_stuff"] = silhouette_stuff
                processed_result["bases"] = basis_out["bases"]
                attns = cat([x.top_feat for x in proposals], dim=0)
                if len(attns)>0:
                    attns = attns.view(-1, len(basis_out["bases"][0][0]), self.attn_size, self.attn_size)
                    attns = F.interpolate(attns, (56, 56), mode='bilinear')
                    processed_result["attns"] = attns[0]
