...
class DatasetMapperWithBasis(DatasetMapper):
    ...
    def __init__(self, cfg, is_train=True):
        ...
        self.contour_stuff       = cfg.MODEL.SEPSNET.CONTOUR_STUFF
        # fmt: on
    ...
    def __call__(self, dataset_dict):
        ...
        if self.one_contour and self.is_train:
            # load contour supervisions
            if self.ann_set == "coco":
                basis_contour_path = (
                    dataset_dict["file_name"]
                    .replace("train2017", "contour_train2017")
                )
            else:
                basis_contour_path = (
                    dataset_dict["file_name"]
                    .replace("leftImg8bit/", "gtFine/")
                    .replace("train/", "contour_train/")
                )
            # change extension to npz
            basis_contour_path = osp.splitext(basis_contour_path)[0] + ".npz"
            basis_contour_gt = np.load(basis_contour_path)["mask"]
            basis_contour_gt = transforms.apply_segmentation(basis_contour_gt)
            basis_contour_gt[basis_contour_gt>0.4] = 1
            basis_contour_gt = torch.as_tensor(basis_contour_gt.astype("long"))
            dataset_dict["basis_contour"] = basis_contour_gt

        if self.contour_stuff and self.is_train:
            # load contour supervisions
            if self.ann_set == "coco":
                contour_stuff_path = (
                    dataset_dict["file_name"]
                    .replace("train2017", "contour_stuff_train2017")
                )
            else:
                contour_stuff_path = (
                    dataset_dict["file_name"]
                    .replace("leftImg8bit/", "gtFine/")
                    .replace("train/", "contour_stuff_train/")
                )
            # change extension to npz
            contour_stuff_path = osp.splitext(contour_stuff_path)[0] + ".npz"
            contour_stuff_gt = np.load(contour_stuff_path)["mask"]
            contour_stuff_gt = transforms.apply_segmentation(contour_stuff_gt)
            contour_stuff_gt[contour_stuff_gt>0.4] = 1
            contour_stuff_gt = torch.as_tensor(contour_stuff_gt.astype("long"))
            dataset_dict["stuff_contour"] = contour_stuff_gt
